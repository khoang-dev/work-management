            ...
            <template #header>
              ...
              <q-item-section>
                ...
                <q-item-label caption>
                  Total Estimate: {{ calculateProjectTime(item) }}
                </q-item-label>
              </q-item-section>
            </template>

            <q-list padding>
              <q-item v-for="(task, taskIndex) in item.tasks" :key="taskIndex">
                <q-item-section side>
                  <q-checkbox
                ...
                <q-item-section>
                  ...
                  <q-item-label caption>
                    Estimate: {{ formatTime(task.timeEstimate) }}
                  </q-item-label>
                </q-item-section>
              </q-item>
            </q-list>
        ...
        <template v-else>
          <q-item>
            ...
            <q-item-section>
              <q-item-label>{{ item.title }}</q-item-label>
              <q-item-label caption>
                Estimate: {{ formatTime(item.timeEstimate) }}
              </q-item-label>
            </q-item-section>
          </q-item>
        </template>
    ...
    <q-dialog v-model="newTaskDialog">
      <q-card style="min-width: 350px">
        ...
        <q-card-actions align="right">
          ...
          <q-btn flat label="Add Task" color="primary" @click="addNewTask" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </div>
